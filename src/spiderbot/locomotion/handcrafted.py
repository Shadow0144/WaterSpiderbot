...
class HandcraftedLocomotionModule(LocomotionModule):
    ...
    def walk_forward(self, delta_time, leg_set):
        ...
        match self.current_state: # Raise set 1 of legs
                case self.LegCycleState.Lifting:
                    coxa_target_angle = math.radians(30)
                    femur_target_angle = math.radians(30)
                    tibia_target_angle = math.radians(45)

                    # Left side
                    #leg_set.left_i_leg.set_leg_targets(-coxa_target_angle, femur_target_angle, tibia_target_angle)
                    leg_set.left_ii_leg.set_leg_targets(-coxa_target_angle, femur_target_angle, tibia_target_angle)
                    #leg_set.left_iii_leg.set_leg_targets(-coxa_target_angle, femur_target_angle, tibia_target_angle)
                    leg_set.left_iv_leg.set_leg_targets(-coxa_target_angle, femur_target_angle, tibia_target_angle)
                    
                    # Right side
                    leg_set.right_i_leg.set_leg_targets(coxa_target_angle, femur_target_angle, tibia_target_angle)
                    #leg_set.right_ii_leg.set_leg_targets(coxa_target_angle, femur_target_angle,  tibia_target_angle)
                    leg_set.right_iii_leg.set_leg_targets(coxa_target_angle, femur_target_angle, tibia_target_angle)
                    #leg_set.right_iv_leg.set_leg_targets(coxa_target_angle, femur_target_angle, tibia_target_angle)

                    if self.timer < 0:
                        self.current_state = self.LegCycleState.Reaching
                        self.timer = 1500.0

                case self.LegCycleState.Reaching:
                    coxa_target_angle = math.radians(-30)
                    femur_target_angle = math.radians(0)
                    tibia_target_angle = math.radians(0)
                ...
                case self.LegCycleState.Planting:
                    coxa_target_angle = math.radians(-30)
                    femur_target_angle = math.radians(0)
                    tibia_target_angle = math.radians(0)
